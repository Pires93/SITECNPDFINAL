    ...
    <div class="container">
      ...
      <div class="col-md-12">
        <div class="form-group">
          <p class="pleft">Deliberações disponíveis: {{ countNumbers() }}</p>
          <input
            id="idsearch"
            type="text"
            class="form-control"
                ...
                <table class="table table-striped table-bordered" style="width:100%">
                    ...
                    <tbody>
                        <tr v-for="(lista, index) in (sortedActivity, filteredList)" :key="lista.id">
                            <td>{{index + 1}}</td> 
                            <td> 
                            <router-link :to="{name: 'deliberaid',params: { id: lista.id}}">  
                              {{lista.titulo}} 
                            </router-link>
                            </td> 
                            <td>{{lista.num_doc}}</td>
...
export default { 
  ...
  created () {
    axios.get('http://127.0.0.1:8000/api/deliberacoes')
      .then(response => {
        this.listas = response.data  
      },)
  },
  mounted() {  
    document.titulo = "Autorizações | CNPD";  
  }, 
